...
import {ArrowUpRight, ChevronDown, ChevronUp, LayoutTemplate, Map, Save, Trash2} from 'lucide-react';
...
// `tab`: 'nav' shows the navigation pages list, 'templates' shows layout templates.
// `docked` renders it as a static left flyout (no drag) for the edit dock.
    ...
    return <div className={`pages-panel${docked ? ' docked' : ''}`} ref={docked ? undefined : panelRef}
        ...
        {showNav && <>
            <div className="pages-list">
                {(pages || []).map((p, i) => <PageRow key={p.path} p={p} route={route} index={i}
                                                      total={(pages || []).length}
